...
func TestRotationAdminPermissions(t *testing.T) {
	...
	// Verify that regular user cannot pass Admin permission check for rotation modifications
	err := permission.LimitCheckAny(userCtx, permission.Admin)
	assert.Error(t, err, "regular user should not have admin permissions for rotations")

	// Verify that admin can pass Admin permission check
	err = permission.LimitCheckAny(adminCtx, permission.Admin)
	assert.NoError(t, err, "admin should have admin permissions for rotations")
}
